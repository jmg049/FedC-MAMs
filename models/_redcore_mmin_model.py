## TODO : Convert to new model format
...
class redcoreMMINModel(BaseModel):
    ...
    def modify_commandline_options(parser, is_train=True):
        parser.add_argument(
            "--input_dim_a", type=int, default=130, help="acoustic input dim"
        )
        parser.add_argument(
            "--input_dim_l", type=int, default=1024, help="lexical input dim"
        )
        parser.add_argument(
            "--input_dim_v", type=int, default=384, help="lexical input dim"
        )
        parser.add_argument(
            "--embd_size_a", default=128, type=int, help="audio model embedding size"
        )
        parser.add_argument(
            "--embd_size_l", default=128, type=int, help="text model embedding size"
        )
        parser.add_argument(
            "--embd_size_v", default=128, type=int, help="visual model embedding size"
        )
        parser.add_argument(
            "--embd_method_a",
            default="maxpool",
        ...
        parser.add_argument(
            "--n_blocks", type=int, default=3, help="number of AE blocks"
        )
        parser.add_argument(
            "--cls_layers",
            type=str,
        ...
        parser.add_argument(
            "--dropout_rate", type=float, default=0.3, help="rate of dropout"
        )
        parser.add_argument(
            "--bn", action="store_true", help="if specified, use bn layers in FC"
        )
        parser.add_argument(
            "--pretrained_path",
            type=str,
            help="where to load pretrained encoder network",
        )
        parser.add_argument(
            "--ce_weight", type=float, default=1.0, help="weight of ce loss"
        )
        parser.add_argument(
            "--mse_weight", type=float, default=1.0, help="weight of mse loss"
        )
        parser.add_argument(
            "--cycle_weight", type=float, default=1.0, help="weight of cycle loss"
        )
        parser.add_argument(
            "--share_weight",
            action="store_true",
    ...
    def __init__(self, opt):
        ...
        self.netAE = ResidualAE(
            AE_layers, opt.n_blocks, AE_input_dim, dropout=0, use_bn=False
        )
        if opt.share_weight:
        ...
        self.feature_dim = 32
        self.netxenc_al2v = ResidualXE(
            AE_layers,
            opt.n_blocks,
            opt.embd_size_a + opt.embd_size_l,
            opt.embd_size_v,
            dropout=0,
            use_bn=False,
        )
        self.netxenc_av2l = ResidualXE(
            AE_layers,
            opt.n_blocks,
            opt.embd_size_a + opt.embd_size_v,
            opt.embd_size_l,
            dropout=0,
            use_bn=False,
        )
        self.netxenc_vl2a = ResidualXE(
            AE_layers,
            opt.n_blocks,
        ...
        self.netCls = FcClassifier(
            opt.embd_size_a + opt.embd_size_l + opt.embd_size_v,
            cls_layers,
            output_dim=opt.output_dim,
            dropout=opt.dropout_rate,
            use_bn=opt.bn,
        )
        self.netCls_a = FcClassifier(
            opt.embd_size_a,
            cls_layers,
            output_dim=opt.output_dim,
            dropout=opt.dropout_rate,
        ...
        if self.isTrain:
            ...
            self.optimizer = torch.optim.Adam(
                paremeters, lr=opt.lr, betas=(opt.beta1, 0.999)
            )
            self.optimizers.append(self.optimizer)

            self.lr_scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(
                self.optimizer, T_max=opt.total_iters
            )
    ...
    def forward(self):
        ...
        # fusion miss
        self.feat_fusion_miss = torch.cat(
            [self.feat_A_miss, self.feat_L_miss, self.feat_V_miss], dim=-1
        )
        ...
        self.gen_a, latent_a = self.netxenc_vl2a(
            torch.cat([self.feat_V_miss, self.feat_L_miss], dim=-1)
        )
        self.gen_l, latent_l = self.netxenc_av2l(
            torch.cat([self.feat_A_miss, self.feat_V_miss], dim=-1)
        )
        self.gen_v, latent_v = self.netxenc_al2v(
            torch.cat([self.feat_A_miss, self.feat_L_miss], dim=-1)
        )

        bs = self.feat_A_miss.shape[0]
        self.feat_A_r = (
            self.A_miss_index.reshape(bs, 1) * self.feat_A_miss
            - (self.A_miss_index.reshape(bs, 1) - 1) * self.gen_a
        )
        self.feat_L_r = (
            self.L_miss_index.reshape(bs, 1) * self.feat_L_miss
            - (self.L_miss_index.reshape(bs, 1) - 1) * self.gen_l
        )
        self.feat_V_r = (
            self.V_miss_index.reshape(bs, 1) * self.feat_V_miss
            - (self.V_miss_index.reshape(bs, 1) - 1) * self.gen_v
        )
        ...
        self.feat_fusion_r = torch.cat(
            [self.feat_A_r, self.feat_L_r, self.feat_V_r], dim=-1
        )
    ...
    def backward(self):
        ...
        kld_fa = (
            ...
            * lambda1
            * torch.sum(
                (1 + self.flogvar_a - self.fmu_a.pow(2) - self.flogvar_a.exp())
                * a_index
            )
            / bs
        )
        kld_fv = (
            ...
            * lambda1
            * torch.sum(
                (1 + self.flogvar_v - self.fmu_v.pow(2) - self.flogvar_v.exp())
                * v_index
            )
            / bs
        )
        kld_fl = (
            ...
            * lambda1
            * torch.sum(
                (1 + self.flogvar_l - self.fmu_l.pow(2) - self.flogvar_l.exp())
                * l_index
            )
            / bs
        )
        ...
        self.lossA_r = (
            self.criterion_mse(self.gen_a * a_index, self.feat_A_miss * a_index) / bs_a
        )
        self.lossV_r = (
            self.criterion_mse(self.gen_v * v_index, self.feat_V_miss * v_index) / bs_v
        )
        self.lossL_r = (
            self.criterion_mse(self.gen_l * l_index, self.feat_L_miss * l_index) / bs_l
        )
        ...
        self.loss_mse = self.mse_weight * (
            self.beta[0] * self.lossA_r
            + self.beta[1] * self.lossV_r
            + self.beta[2] * self.lossL_r
        )
        ...
        loss = (
            self.loss_CE
            + (kld_fa + kld_fv + kld_fl)
            + (self.loss_CE_a + self.loss_CE_v + self.loss_CE_l)
            + self.loss_mse
        )
        # print('loss:', loss)
        ...
        for model in self.model_names:
            torch.nn.utils.clip_grad_norm_(
                getattr(self, "net" + model).parameters(), 1.0
            )
